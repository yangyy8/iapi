      ...
      <el-row type="flex" style="height:100%">
        <el-col :span="22" class="br flex-c pr-20">
          ...
          <el-row align="center" :gutter="2">
            ...
            <el-col :sm="24" :md="12" :lg="6" class="input-item">
              <span class="input-text">旅客订票号：</span>
    ...
    <div class="middle">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            {{scope.row.CHECKRESULT|fiftercheck}}
          </template>
        </el-table-column>
        <el-table-column
          prop="FLIGHTTYPE"
        ...
        <el-table-column
          ...
          min-width="150">
          <template slot-scope="scope">
            <el-button type="text"  class="a-btn" title="详情" size="mini" icon="el-icon-tickets" name="ryxxcx_zh_detail" @click="details(scope.row)"></el-button>
            <el-button type="text"  class="a-btn" title="同值机人员" icon="el-icon-news" name="ryxxcx_zh_same_checker" @click="$router.push({name:'QueryGLRY',query:{row:scope.row,page:1,title:'关联人员查询'}})"></el-button>
            <el-button type="text"  class="a-btn" title="同订票人员" icon="el-icon-mobile-phone" name="ryxxcx_zh_same_booker" @click="$router.push({name:'QueryGLRY',query:{row:scope.row,page:0,title:'关联人员查询'}})"></el-button>
            <el-button type="text"  class="a-btn" title="临近座位" icon="el-icon-rank" name="ryxxcx_zh_near_seat" @click="$router.push({name:'QueryGLRY',query:{row:scope.row,page:3,title:'关联人员查询'}})"></el-button>
            <el-button type="text"  class="a-btn" title="航班座位" icon="el-icon-document" name="ryxxcx_zh_flight_seat" @click="seat(scope.row)"></el-button>
         </template>
        </el-table-column>
      </el-table>
      ...
      <div class="middle-foot">
        <div class="page-msg">
          ...
          <div v-loading="loading" class="loadingtext">
            共{{totalResult}}条
          </div>
        </div>
        <el-pagination
...
export default {
  ...
  data(){
    return{
      ...
      PNR_TID0:'',
      PNR_TKTNUMBER0:'',
      PNR_TRAVELLER_SURNAME_TIF0:'',
      PNR_TRAVELLER_GIVEN_NAME_TIF0:'',
      dform:{},
      dpform:{},
  ...
  methods:{
    ...
    details(i){
      ...
      this.CHK_SERIAL0 = i.CHK_SERIAL;
      this.PNR_TID0 = i.PNR_TID;
    ...
    pageSizeChange(val) {//显示条数，调用
      if(this.batchFlag == 0){
        if(this.orderState == 1){//排序
          this.sortChange(this.orderHc)
        }else if(this.orderState == 0){
          this.getList(this.currentPage,val,this.cdt);
        }
      }else if(this.batchFlag == 1){//批量导入
        if(this.orderState == 1){//排序
          this.sortChange(this.orderHc)
        }else if(this.orderState == 0){
          this.getList(this.currentPage,val,this.cdt1,1);
        }
      }
    },
    handleCurrentChange(val){
      if(this.batchFlag == 0){
        ...
        if(this.orderState == 1){
          ...
          this.getList(val,this.showCount,this.cdt1,1);
        }
      }
    },
    //===========================打印导出================================
