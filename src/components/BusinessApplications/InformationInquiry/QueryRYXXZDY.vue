...
export default {
  ...
  methods:{
    ...
    sortChange(column){
      ...
      this.orderIapiHcZDY=column;
      let p={
        'orders':[this.orderIapiHcZDY.prop],
    ...
    sortChangePnr(column){
      ...
      this.orderPnrStateZDY=1;
      this.orderPnrHcZDY=column;
      let p={
    ...
    pageSizeChange(val){//显示条数
      if(this.bigBase==7){
        if(this.orderIapiStateZDY==0){
          this.selfQueryList(this.currentPage,val)
        }else if(this.orderIapiStateZDY==1){
          this.sortChange(this.orderIapiHcZDY)
        }
      }else if(this.bigBase==8){
        if(this.orderPnrStateZDY==0){
          this.selfQueryListPnr(this.currentPagePnr,val);
        }else if(this.orderPnrStateZDY==1){
          this.sortChangePnr(this.orderPnrHcZDY);
        }
      }
    },
    ...
    selfS(){
      if(this.bigBase == 7){
        for(var i=0;i<this.selfRows.length;i++){
          if(this.selfRows[i].attribute!=''){
            if(this.selfRows[i].operator==''){
              this.$message({
                message: this.nameChange(this.selfRows[i].attribute,this.selfNature)+'的运算符不能为空',
                type: 'warning'
              });
              return
            }
            if(this.selfRows[i].atype==''){
              this.$message({
                message: this.nameChange(this.selfRows[i].attribute,this.selfNature)+'的值不能为空',
                type: 'warning'
              });
              return
            }
            if(this.selfRows[i].relation==''&&(i!=0)){
              this.$message({
                message: this.nameChange(this.selfRows[i].attribute,this.selfNature)+'的逻辑关系不能为空',
                type: 'warning'
              });
              return
            }
        ...
        for(var i=0;i<this.selfRowsPnr.length;i++){
          if(this.selfRowsPnr[i].attribute!=''){
            if(this.selfRowsPnr[i].operator==''){
              this.$message({
                message: this.nameChange(this.selfRowsPnr[i].attribute,this.selfNaturePnr)+'的运算符不能为空',
                type: 'warning'
              });
              return
            }
            if(this.selfRowsPnr[i].atype==''){
              this.$message({
                message: this.nameChange(this.selfRowsPnr[i].attribute,this.selfNaturePnr)+'的值不能为空',
                type: 'warning'
              });
              return
            }
            if(this.selfRowsPnr[i].relation==''&&(i!=0)){
              this.$message({
                message: this.nameChange(this.selfRowsPnr[i].attribute,this.selfNaturePnr)+'的逻辑关系不能为空',
                type: 'warning'
              });
              return
            }
