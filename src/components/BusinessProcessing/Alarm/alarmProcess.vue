        ...
        <div class="middle-content1 position">
          ...
          <el-table
            ...
            <el-table-column
              ...
              width="70">
              <template slot-scope="scope">
                <el-button v-if="scope.row.status==1" type="text" class="a-btn" icon="el-icon-view" title="查看" @click="$router.push({name:'infoComparison',query:{row:scope.row,status:scope.row.status,AlarmType:$route.query.AlarmType,dh:scope.row.dh,isZDGZ:$route.query.isZDGZ,NameListType:scope.row.alarmTypeNum,eventserial:eventserial,iapiSerial:iapiMap.iapiSerial,instructNew:iapiMap.instructNew,visaNo:scope.row.visano,inOut:iapiMap.flightType,nationAndPass:scope.row.nationalityCode+scope.row.passportno}})"></el-button>
                <el-button v-else type="text" class="a-btn" icon="el-icon-edit-outline" title="甄别"  @click="$router.push({name:'infoComparison',query:{row:scope.row,status:scope.row.status,AlarmType:$route.query.AlarmType,dh:scope.row.dh,isZDGZ:$route.query.isZDGZ,NameListType:scope.row.alarmTypeNum,eventserial:eventserial,iapiSerial:iapiMap.iapiSerial,instructNew:iapiMap.instructNew,visaNo:scope.row.visano,inOut:iapiMap.flightType,nationAndPass:scope.row.nationalityCode+scope.row.passportno}})"></el-button>
